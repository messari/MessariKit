...
import type { getNewsFeedParameters, getNewsFeedAssetsParameters, getNewsSourcesParameters, NewsAsset, Document, Source } from "@messari/sdk-types";
...
async function main() {
  try {
    ...
    const paginatedArticles = await client.news.getNewsFeedPaginated(newsFeedParams);

    // Display pagination metadata
    console.log("Pagination metadata:", paginatedArticles.metadata);
    console.log(`Page ${paginatedArticles.metadata?.page} of ${paginatedArticles.metadata?.totalPages}`);
    ...
    if (paginatedArticles.hasNextPage) {
      ...
      if (page2.hasNextPage) {
        ...
        console.log(`Page ${page3.metadata?.page} of ${page3.metadata?.totalPages}`);
    ...
    const paginatedAssets = await client.news.getNewsFeedAssetsPaginated(assetsParams);

    // Display pagination metadata
    console.log("Pagination metadata:", paginatedAssets.metadata);
    console.log(`Page ${paginatedAssets.metadata?.page} of ${paginatedAssets.metadata?.totalPages}`);
    ...
    const paginatedSources = await client.news.getNewsSourcesPaginated(sourcesParams);

    // Display pagination metadata
    console.log("Pagination metadata:", paginatedSources.metadata);
    console.log(`Page ${paginatedSources.metadata?.page} of ${paginatedSources.metadata?.totalPages}`);
...
function displayArticles(articles: Document[]) {
  ...
  articles.forEach((article: Document, index: number) => {
    ...
    if (article.assets && article.assets.length > 0) {
      console.log(`   Mentioned assets: ${article.assets.map((a: NewsAsset) => a.name).join(", ")}`);
    }
